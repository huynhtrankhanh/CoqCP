...
class CoqCPASTTransformer {
    ...
    transform() {
        ...
        for (const node of this.ast.body) {
            ...
            if (node.expression.callee.name === 'procedure') {
                ...
                const body: Instruction[] = this.transformBodyNode(
                    bodyNode.body
                )
    ...
    private processNode(node: ExtendNode<ESTree.Node>): ValueType {
        if (
            ...
            if (
                ...
                throw new ParseError(
                    "left hand side can't be a literal. " +
                        formatLocation(node.loc)
                )
            }
            return {
        ...
        switch (name) {
            ...
            case 'store': {
                ...
                const tuples = args[2].elements.map((node) => {
                    ...
                    if (node.type === 'SpreadElement') {
                        throw new ParseError(
                            'spread syntax not supported, ' +
                                formatLocation(location)
                        )
                    }
                    return this.processNode(
                        node as ExtendNode<ESTree.Expression>
                    )
                })
            ...
            case 'range': {
                ...
                if (parameter.type !== 'Identifier') {
                    throw new ParseError(
                        "this parameter isn't recognized. " +
                            formatLocation(parameter.loc)
                    )
                }
    ...
    private transformBodyNode(
        ...
        for (const statement of bodyNode.body) {
            if (statement.type === 'IfStatement') {
                ...
                if (alternate.type !== 'BlockStatement') {
                    if (alternate.loc === undefined || alternate.loc === null) {
                        throw new ParseError(
                            'must be a block statement. ' + statement.loc
                        )
                    }
                    throw new ParseError(
                        'must be a block statement. ' +
                            formatLocation(alternate.loc)
                    )
                }
                instructions.push({
                    type: 'condition',
            ...
            instructions.push(
                this.processNode(statement.expression) as Instruction
            )
        }
