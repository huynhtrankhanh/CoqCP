...
Lemma mergingLogic3 (dsu : list Slot) (hL : length dsu = 100) (hL1 : Z.to_nat (dsuLeafCount dsu) = length dsu) (h1 : noIllegalIndices dsu) (h2 : withoutCyclesN dsu (length dsu)) (a b : Z) (x y : Z) (hLeX : Z.le 0 x) (hLtX : Z.lt x 100) (hLeY : Z.le 0 y) (hLtY : Z.lt y 100) (hdiff : ancestor dsu (length dsu) (Z.to_nat a) <> ancestor dsu (length dsu) (Z.to_nat b)) (hLe1 : Z.le 0 a) (hLt1 : Z.lt a 100) (hLe2 : Z.le 0 b) (hLt2 : Z.lt b 100) tree1 tree2 (htree1 : nth (ancestor dsu (length dsu) (Z.to_nat a)) (pathCompress (pathCompress dsu (length dsu) (Z.to_nat x) (ancestor dsu (length dsu) (Z.to_nat x))) (length dsu) (Z.to_nat y)
              (ancestor dsu (length dsu) (Z.to_nat y))) (Ancestor Unit) = Ancestor tree1) (htree2 : nth (ancestor dsu (length dsu) (Z.to_nat b)) (pathCompress
              (pathCompress dsu (length dsu) (Z.to_nat x)
                 (ancestor dsu (length dsu) (Z.to_nat x))) 
              (length dsu) (Z.to_nat y)
              (ancestor dsu (length dsu) (Z.to_nat y))) (Ancestor Unit) = Ancestor tree2) whatever whatever2 ignored : invokeContractAux (repeat 1%Z 20) (repeat 0%Z 20) 0 (stateAfterInteractions
     ignored (dsuScore dsu)) (stateAfterInteractions
     ignored (dsuScore dsu)) [x; y] 1 arrayIndex0
  arrayIndexEqualityDecidable0 (arrayType arrayIndex0 environment0)
  (λ _0 : arrayIndex0,
     match
...
Lemma runUnite3 (dsu : list Slot) (hL : length dsu = 100) (hL1 : Z.to_nat (dsuLeafCount dsu) = length dsu) (h1 : noIllegalIndices dsu) (h2 : withoutCyclesN dsu (length dsu)) (a b : Z) (hLe1 : Z.le 0 a) (hLt1 : Z.lt a 100) (hLe2 : Z.le 0 b) (hLt2 : Z.lt b 100) ignored : invokeContractAux (repeat 1%Z 20) (repeat 0%Z 20) 0 (stateAfterInteractions ignored
  (dsuScore
  dsu))
  (stateAfterInteractions ignored
  (dsuScore
...
Proof.
  ...
  - unfold numberLocalGet at 1. rewrite -!bindAssoc pushNumberGet2.
    ...
    pose proof pathCompressPreservesWithoutCycles (pathCompress dsu (length dsu) (Z.to_nat a)
     ...
     rewrite pathCompressPreservesLength !pathCompressPreservesAncestorLength in hs; try (assumption || lia).
     pose proof mergingLogic3 hL hL1 h1 h2 (ltac:(lia) : (0 <= a)%Z) ltac:(lia) (ltac:(lia) : (0 <= b)%Z) ltac:(lia) (ltac:(lia) : ancestor dsu (length dsu) (Z.to_nat b) ≠ ancestor dsu (length dsu) (Z.to_nat a)) ltac:(lia) ltac:(lia) ltac:(lia) ltac:(lia) hrnode hlnode as st.
     rewrite st. apply (ltac:(intros T U mm ma mb mc; subst ma; reflexivity) : forall (T U : Type) (g : T) (a b : U), a = b -> Some (g, a) = Some (g, b)).
     apply functional_extensionality_dep. intro x. unfold update. unfold stateAfterInteractions. repeat case_decide; try easy.
     { rewrite performMergeScore; try (assumption || lia || (rewrite !pathCompressPreservesLength; apply ancestorLtLength; (assumption || lia))).
       rewrite !pathCompressPreservesScore.
   ...
   + rewrite -!bindAssoc !leftIdentity.
     rewrite pathCompressPreservesLength !pathCompressPreservesAncestorLength in hs; try (assumption || lia).
     pose proof mergingLogic3 hL hL1 h1 h2 (ltac:(lia) : (0 <= a)%Z) ltac:(lia) (ltac:(lia) : (0 <= b)%Z) ltac:(lia) (ltac:(lia) : ancestor dsu (length dsu) (Z.to_nat a) ≠ ancestor dsu (length dsu) (Z.to_nat b)) ltac:(lia) ltac:(lia) ltac:(lia) ltac:(lia) hlnode hrnode as st.
     rewrite st. apply (ltac:(intros T U mm ma mb mc; subst ma; reflexivity) : forall (T U : Type) (g : T) (a b : U), a = b -> Some (g, a) = Some (g, b)).
     apply functional_extensionality_dep. intro x. unfold update. unfold stateAfterInteractions. repeat case_decide; try easy.
     { rewrite performMergeScore; try (assumption || lia || (rewrite !pathCompressPreservesLength; apply ancestorLtLength; (assumption || lia))).
       rewrite !pathCompressPreservesScore.
...
Lemma outOfBoundsInteractionNAC (a b : Z) (hOOB : Z.le 100 a) (hUB : Z.lt a 256) (dsu : list Slot) (hL : length dsu = 100) : invokeContract (repeat 1%Z 20) (repeat 0%Z 20) 0%Z (stateAfterInteractions (fun x => match x with | arraydef_0__result => [0%Z] | arraydef_0__hasBeenInitialized => [1%Z] | arraydef_0__dsu => convertToArray dsu end) (dsuScore dsu)) (stateAfterInteractions (fun x => match x with | arraydef_0__result => [0%Z] | arraydef_0__hasBeenInitialized => [1%Z] | arraydef_0__dsu => convertToArray dsu end) (dsuScore dsu)) [a; b] 1 = Some ([], (stateAfterInteractions (fun x => match x with | arraydef_0__result => [0%Z] | arraydef_0__hasBeenInitialized => [1%Z] | arraydef_0__dsu => convertToArray dsu end) (dsuScore dsu))).
Proof.
...
Proof.
  ...
  - rewrite firstInteraction; try lia.
    ...
    { rewrite w. rewrite unitePreservesLeafCount; (assumption || easy). }
    clear ah jh w h1 h2 h3 h4 hv a b.
    revert dsu st1 st2 hL hC.
    induction tail as [| head tail IH].
    + easy.
    + intros dsu st1 st2 hL hC.
 ...
 = repeat 1%Z 20) (ltac:(easy) : [0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z;
     0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z] = repeat 0%Z 20).
        unfold invokeContract. rewrite (ltac:(easy) : stateAfterInteractions _ _ (repeat 0%Z 20) = BlockchainContract _ _ _ _ _ _). unfold funcdef_0__main at 2. rewrite !leftIdentity. unfold retrieve at 1. rewrite <- !bindAssoc.
        rewrite pushDispatch2. rewrite unfoldInvoke_S_Retrieve. case_decide as jj; [| simpl in jj; lia]. rewrite !leftIdentity. unfold readByte.
        rewrite pushDispatch2 unfoldInvoke_S_ReadByte !leftIdentity.
        case_decide as uu; [| simpl in uu; lia].
        rewrite -!bindAssoc pushDispatch2 unfoldInvoke_S_ReadByte.
        case_decide as uU; [| simpl in uU; lia].
        rewrite !leftIdentity (ltac:(easy) : nth (Z.to_nat 0) [a; b] 0%Z = a) (ltac:(easy) : nth (Z.to_nat 1) [a; b] 0%Z = b).
        pose proof mN a b ltac:(simpl; left; reflexivity) as ju.
        assert (alww : (update (update (fun=> 0%Z) vardef_0__unite_u a)vardef_0__unite_v b) = fun jj => match jj with | vardef_0__unite_u => a | vardef_0__unite_v => b | vardef_0__unite_z => 0%Z end).
        { apply functional_extensionality_dep. intro wa. destruct wa; easy. }
        rewrite alww. unfold store.
        rewrite (runUnite3 (ltac:(lia) : length dsu = 100) ltac:(lia) st2 ltac:(rewrite hL; assumption) (ltac:(lia) : (0 <= a)%Z) ltac:(lia) (ltac:(lia) : (0 <= b)%Z) ltac:(lia)).
        apply IH.
        { intros j1 j2 j3. apply mN. { simpl. tauto. } }
        { rewrite <- hL. apply unitePreservesWithoutCycles; rewrite -> hL in *; assumption. }
        { apply unitePreservesNoIllegalIndices. assumption. }
        { rewrite unitePreservesLength. assumption. }
        { rewrite unitePreservesLeafCount; assumption. }
 ...
 = repeat 1%Z 20) (ltac:(easy) : [0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z;
     0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z] = repeat 0%Z 20).
          ...
          eapply (IH _ _ st1 st2 hL hC). Unshelve. intros t y p. apply mN. simpl. tauto. }
        { rewrite (ltac:(easy) : [1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z;
     1%Z; 1%Z; 1%Z; 1%Z; 1%Z; 1%Z]
 = repeat 1%Z 20) (ltac:(easy) : [0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z;
     0%Z; 0%Z; 0%Z; 0%Z; 0%Z; 0%Z] = repeat 0%Z 20).
