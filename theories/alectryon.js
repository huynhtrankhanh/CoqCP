...
;(function (Alectryon) {
  ;(function (slideshow) {
    function anchor(sentence) {
      return '#' + sentence.id
    }

    function current_sentence() {
      return slideshow.sentences[slideshow.pos]
    }
    ...
    function scroll(sentence) {
      ...
      var parent_box = parent
          ? rect(parent)
          : { y: 0, height: window.innerHeight },
      ...
      if (
        sentence_y < 0.1 * parent_box.height ||
        sentence_y > 0.7 * parent_box.height
      ) {
        ;(parent || window).scrollBy(
          0,
          Math.max(
            sentence_y - 0.5 * parent_box.height,
    ...
    function navigate(pos, inhibitScroll) {
      ...
      var sentence = current_sentence()
      highlight(sentence)
      if (!inhibitScroll) scroll(sentence)
    }
    ...
    function onkeydown(e) {
      ...
      if (e.ctrlKey || e.metaKey) {
        if (e.keyCode == keys.ARROW_UP) slideshow.previous()
        else if (e.keyCode == keys.ARROW_DOWN) slideshow.next()
        else return
      } else {
    ...
    function toggleHighlight(idx) {
      if (highlighted(idx)) unhighlight()
      else navigate(idx, true)
    }
    ...
    function init() {
      ...
      slideshow.pos = -1
      slideshow.sentences = Array.from(
        document.getElementsByClassName('alectryon-sentence')
      )
