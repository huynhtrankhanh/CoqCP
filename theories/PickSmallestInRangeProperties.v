...
From CoqCP Require Import ListRange Foldl Comparator SelectionSort.
...
Proof.
  remember (compare (nth (i + S delta) l default) (nth (pickSmallestInRange default compare i (i + delta) l) l default)) as H.
  destruct H.
...
Proof.
  revert inBetween hInBetween. induction delta; intros.
  ...
  - rewrite Nat.add_succ_r. unfold pickSmallestInRange in *. rewrite range2LeftAdd, foldl_app, ?foldlSingleton, pickSmallestInRangeFold. rewrite pickSmallestInRangeFold in IHdelta.
    remember (compare A comparator (nth (S (i + delta)) l default) (nth (pickSmallestInRange default (compare A comparator) i (i + delta) l) l default)) as H. destruct H.
    + assert (hSplit : inBetween = S delta \/ inBetween <= delta). { lia. }
      destruct hSplit.
      * rewrite H, Nat.add_succ_r. apply (irreflexive _ comparator).
      * pose proof IHdelta inBetween H. symmetry in HeqH. rewrite <- Is_true_true in HeqH. pose proof asymmetry _ _ _ _ HeqH as H1. apply (negativelyTransitive _ _ _ _ H0 H1).
    + assert (hSplit : inBetween = S delta \/ inBetween <= delta). { lia. }
      destruct hSplit.
