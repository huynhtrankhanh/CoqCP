...
procedure(
  ...
  () => {
    ...
    range(get('array_size'), (i) => {
      ...
      range(8, (byte_index) => {
        set(
          'current_int',
          get('current_int') +
            coerceInt64(retrieve(i * 8 + byte_index)) *
              (1 << ((7 - byte_index) * 8))
        )
      })
    ...
    range(get('array_size'), (i) => {
      // Outer loop
      range(get('array_size') - get('i') - 1, (j) => {
        // Inner loop
        if (
          !less(
            retrieve('data', get('j') + 1)[0],
            retrieve('data', get('j'))[0]
          )
        ) {
          // if data[j] > data[j+1] in unsigned comparison (ascending order)
