...
procedure('readSigned64BitInteger', { tmpChar: int64, result: int64, sign: int64 }, () => {
  set('result', 0)
  set('sign', 1)
  ...
  range(20, (i) => {
    ...
    if (get('result') == 0 && get('sign') == 1) {
      if (get('tmpChar') == 43) {  // '+' sign
        set('sign', 1)
        ;('continue')
      } else if (get('tmpChar') == 45) {  // '-' sign
        set('sign', -1)
        ;('continue')
      } else if (less(get('tmpChar'), 48) || !less(get('tmpChar'), 58)) {
